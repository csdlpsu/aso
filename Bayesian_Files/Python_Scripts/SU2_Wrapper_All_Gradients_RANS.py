...
(Mach, Re, AoA) as well as FFD control points.
...
To run:
python SU2_Wrapper_All_Gradients.py -mcfg "config_NACA0012.cfg" -mesh "mesh_NACA0012.su2" -dv "DV_VALUE.npy" -np 1 -si 0 -cmin -10 -gflag "T" -mach 0.83 -re_no 6500000 -aoa 3.06
"""
...
class ExperimentDesignWrapper():
    ...
    def write_config_file_DVs(self,config_file,dv_values, num_processes, minval, aoa, re_num, mach_num):
...
def objective_function(dv_value, working_cfg, num_processes, history_file, minval,
    ...
    print(dv_value)
    print(minval)
    EDW.write_config_file_DVs(working_cfg,dv_value, num_processes, minval, aoa, re_num, mach_num) # Writing the DV_VALUE to main
    ...
    th_df=pd.read_csv('of_func.csv', header=0, usecols=["AIRFOIL_THICKNESS"])
    th_data=th_df.head()
    th_num=th_data.to_numpy()
    th_reshape=th_num.reshape(1, -1)
    thickness=th_reshape[0]
    np.save("Thickness", thickness)
    ...
    if grad_flag == "T":
       ...
       th_grad_df=pd.read_csv('of_grad.csv', header=0, usecols=["AIRFOIL_THICKNESS"])
       ...
       np.save("Thickness_Gradients", thickness_grads)
...
parser.add_argument("-reno",  "--REYNOLDS_NO", type=float, default=6500000, help="Reynolds Number")
...
objective_function(dv_value=dv_value, working_cfg=working_cfg, num_processes=num_processes,
                   ...
                   re_num=re_num, mach_num=mach_num)
