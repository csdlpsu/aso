...
"""
A script that uses Bayesian Optimization to minimize drag of the ONERA M6 constrained problem with 5 Thickness inequality constraints and an equality constraint on lift.
...
- Constraint_1.npy : History of the Thickness1 Constraint
- Constraint_2.npy : History of the Thickness2 Constraint
- Constraint_3.npy : History of the Lift Constraint
- Constraint_4.npy : History of the Thickness3 Constraint
- Constraint_5.npy : History of the Thickness4 Constraint
- Constraint_6.npy : History of the Thickness5 Constraint
...
parser.add_argument("-mcfg",  "--main_cfg",   type=str,   default='ffd_oneram6_Euler_12pts.cfg',   help='Path to the main cfg file')
parser.add_argument("-mesh",  "--mesh_su2",   type=str,   default="mesh_ONERAM6_inv_FFD.su2", help="Path to the SU2 mesh file")
parser.add_argument("-dim",   "--dimensions", type=int,   default=12,                        help='Number of control points for the FFD box')
parser.add_argument("-bsize", "--bound_size", type=float, default=0.00025,                    help="Bound max/min of the control points")
...
def eval_objective(dv_value,i): #This is an objective function that gets the value at the point
    ...
    objective_function_command=['python', 'SU2_Wrapper_All_Gradients_RANS.py','-mcfg', 'ffd_oneram6_Euler_12pts.cfg','-mesh', \
'mesh_ONERAM6_inv_FFD.su2','-dv', 'DV_VALUE.npy','-np', '48','-si', '0','-cmin','-8','-gflag',"F",'-mno','0.8395','-reno','6500000','-aoa','3.06']
    ...
    subprocess.run(["cp", "Station1_Thickness.npy", "Station1_Thickness"+str(i)+".npy"])
    subprocess.run(["cp", "Station2_Thickness.npy", "Station2_Thickness"+str(i)+".npy"])
    subprocess.run(["cp", "Station3_Thickness.npy", "Station3_Thickness"+str(i)+".npy"])
    subprocess.run(["cp", "Station4_Thickness.npy", "Station4_Thickness"+str(i)+".npy"])
    subprocess.run(["cp", "Station5_Thickness.npy", "Station5_Thickness"+str(i)+".npy"])
    thickness1=np.load("Station1_Thickness"+str(i)+".npy") #Getting thickness1
    thickness2=np.load("Station2_Thickness"+str(i)+".npy") #Getting thickness2
    thickness3=np.load("Station3_Thickness"+str(i)+".npy") #Getting thickness3
    thickness4=np.load("Station4_Thickness"+str(i)+".npy") #Getting thickness4
    thickness5=np.load("Station5_Thickness"+str(i)+".npy") #Getting thickness5
    ...
    return drag.item(),cons1,cons2,cons3,cons4,cons5,cons6
    

#Thickness1 Constraint
def c1(x):
    return .078 - x #thickness greater than .078
#Thickness2 Constraint
def c2(x):
    return .072 - x #thickness greater than .072
#Thickness3 Constraint
def c4(x):
    return .066 - x #thickness greater than .066
#Thickness4 Constraint
def c5(x):
    return .061 - x #thickness greater than .061
#Thickness5 Constraint
def c6(x):
    return .055 - x #thickness greater than .055

#Lift Equality Constraint C_l = 0.292
def c3(x):
    print("Constraint 3 evaluation:\n")
    print(f"Lift is {x} \n")
    error=abs(x-.292)
    print(f"Error is {error} \n")
    print(f"Error-tolerance {error-tolerance} \n")
    return error - tolerance

#Turb setup, as shown in the turbo commented document
@dataclass
class ScboState:
    dim: int
    batch_size: int
...
#Generating a new batch of points to look at 
def generate_batch(
    state,
    model,  # GP model
...
c6_temp=[]
for x in train_X:
    drag,thick_con1,thick_con2,lift_con,thick_con3,thick_con4,thick_con5=eval_objective(x,i)
    train_y_temp.append(drag)
    c1_temp.append(thick_con1)
    c2_temp.append(thick_con2)
    c3_temp.append(lift_con)
    c4_temp.append(thick_con3)
    c5_temp.append(thick_con4)
    c6_temp.append(thick_con5)
    i+=1
...
print(f"\nC3 is:\n{C3}\n")
C4 = torch.tensor(c4_temp, **tkwargs).unsqueeze(-1) #Getting the third constraint
print(f"\nC3 is:\n{C4}\n")
C5 = torch.tensor(c5_temp, **tkwargs).unsqueeze(-1) #Getting the third constraint
print(f"\nC3 is:\n{C4}\n")
C6 = torch.tensor(c6_temp, **tkwargs).unsqueeze(-1) #Getting the third constraint
print(f"\nC3 is:\n{C6}\n")
...
lift_i=40
while not state.restart_triggered and maximum_iterations<100:  # Run until TuRBO converges (AKA when restart_triggered is True, the solution is converged)
    ...
    c4_model = get_fitted_model(train_X, C4) #Getting the constraint 2 GP model

    c5_model = get_fitted_model(train_X, C5) #Getting the constraint 2 GP model

    c6_model = get_fitted_model(train_X, C6) #Getting the constraint 2 GP model
    ...
    X_next = generate_batch(
        ...
        X=train_X,
        Y=train_Y,
        C=torch.cat((C1, C2, C3, C4, C5, C6), dim=-1),
    ...
    c6_next_temp=[]
    for x in X_next:
        drag,thick_con1,thick_con2,lift_con,thick_con3,thick_con4,thick_con5=eval_objective(x,lift_i)
        y_next_temp.append(drag)
        c1_next_temp.append(thick_con1)
        c2_next_temp.append(thick_con2)
        c3_next_temp.append(lift_con)
        c4_next_temp.append(thick_con3)
        c5_next_temp.append(thick_con4)
        c6_next_temp.append(thick_con5)
        lift_i+=1
    ...
    C4_next = torch.tensor(c4_next_temp, dtype=dtype, device=device).unsqueeze(-1) #Getting the first constraint
    C5_next = torch.tensor(c5_next_temp, dtype=dtype, device=device).unsqueeze(-1) #Getting the second constraint
    C6_next = torch.tensor(c6_next_temp, dtype=dtype, device=device).unsqueeze(-1) #Getting the third constraint
    
    # Adjust dimensions for consistency among constraints
    if C1_next.dim()>2:
        C1_next = C1_next.squeeze(-1)
    if C2_next.dim()>2:
        C2_next = C2_next.squeeze(-1)
    if C4_next.dim()>2:
        C4_next = C4_next.squeeze(-1)
    if C5_next.dim()>2:
        C5_next = C5_next.squeeze(-1)
    if C6_next.dim()>2:
        C6_next = C6_next.squeeze(-1)

    
    C_next = torch.cat([C1_next, C2_next, C3_next, C4_next, C5_next, C6_next], dim=-1) #Putting C1 C2 and C3 together
    ...
    C4 = torch.cat((C4, C4_next), dim=0) #concatenating the new training data for constraint 1
    C5 = torch.cat((C5, C5_next), dim=0) #concatenating the new training data for constraint 2
    C6 = torch.cat((C6, C6_next), dim=0) #concatenating the new training data for constraint 3
    ...
    np.save("Constraint_4.npy",C4)
    np.save("Constraint_5.npy",C5)
    np.save("Constraint_6.npy",C6)
